...
class FetchValuePart(beam.DoFn):
...
def run():
    ...
    join_data = (pipelines_dictionary
                 ...
                 | 'Fetching Values' >> beam.ParDo(FetchValuePart())
                 | 'Again Convert to List' >> beam.combiners.ToList()
                 | 'Formatting query' >> beam.Map(
                            lambda x:read_query.format(','.join(map(lambda x:'"' + str(x) + '"',x))))
                 | 'querying' >> beam.io.Read(beam.io.BigQuerySource(
